...
int main(int argc, char* argv[])
{
    ...
    if (!data.pipeline || !data.source || !data.vconvert || !data.vsink ||
        !data.aconvert || !data.resample || !data.asink) {
    ...
    gst_bin_add_many(GST_BIN(data.pipeline), data.source, data.aconvert, data.resample, data.asink, NULL);
    if (!gst_element_link_many(data.aconvert, data.resample, data.asink, NULL)) {
        g_printerr("Elements could not be linked.\n");
        return -1;
    }
    
    gst_bin_add_many(GST_BIN(data.pipeline), data.vconvert, data.vsink, NULL);
    if (!gst_element_link_many(data.vconvert, data.vsink, NULL)) {
        g_printerr("Elements could not be linked (video branch).\n");
        return -1;
    }

    /* Set the URI to play */
    g_object_set(data.source, "uri", "file:///home/aniu/Videos/Forrest_Gump_IMAX.mp4", NULL);
